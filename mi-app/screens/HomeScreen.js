...
function HomeScreen({
  ...
  useEffect(() => {
    // Configurar automáticamente el día actual cuando se pase showToday desde la navegación
    if (route.params?.showToday) {
      const today = new Date().toLocaleString("es-ES", { weekday: "long" }).toLowerCase();
      setSelectedDay(today);
    }
  }, [route.params?.showToday]);
  ...
  const toggleHabitCompletion = (index) => {
    ...
    const habitIndex = habits.findIndex(
      (habit) => habit.days?.includes(selectedDay) && habit === filteredHabits[index]
    );
    if (habitIndex >= 0) {
